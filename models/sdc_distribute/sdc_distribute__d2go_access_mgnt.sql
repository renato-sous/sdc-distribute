...
{%- if var('sdc_distribute__external_access_mgnt',none) is not none -%}
select
    user_or_role,
    view_name,
    active
from
    {{ var('sdc_distribute__external_access_mgnt') }}
{%- else -%}
select
    axx.grantee user_or_role,
...
where
    axx.dist_database_name = 'DISTRIBUTE' and
    axx.enabled = 'Y' and
    obj.enabled = 'Y'
{%- endif -%}
